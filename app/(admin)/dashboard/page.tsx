...
function QueueCard({
  ...
  return (
    <Link href={href} className="app-queue rounded-xl border border-[#2a4063] bg-[#13233f] p-5 hover:border-orange-400">
      <p className="app-label text-xs font-black uppercase tracking-wide text-orange-200">{label}</p>
      <p className="app-value mt-2 text-3xl font-black text-white">{value}</p>
...
function DashboardPanel({
  ...
  return (
    <section className="app-panel rounded-xl border border-[#2a4063] bg-[#111f38]">
      <div className="app-panel-head flex items-center justify-between border-b border-[#2a4063] px-5 py-4">
      ...
      <div className="app-panel-body p-5">{children}</div>
    </section>
  );
}
...
export default async function DashboardPage() {
  ...
  return (
    <div className="app-page mx-auto grid max-w-7xl gap-6">
      <header className="app-hero flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="app-actions flex flex-wrap gap-2">
          <Link className="app-btn app-btn-primary inline-flex items-center gap-2 rounded-lg bg-orange-500 px-4 py-2 text-sm font-black text-slate-950 hover:bg-orange-400" href="/work-orders/new">
            <ClipboardPlus className="size-4" />
            New work order
          </Link>
          <Link className="app-btn app-btn-secondary inline-flex items-center gap-2 rounded-lg border border-slate-500 bg-[#13233f] px-4 py-2 text-sm font-black text-white hover:border-orange-300" href="/job-intake">
            <Inbox className="size-4" />
            Job Intake
          </Link>
          <Link className="app-btn app-btn-secondary inline-flex items-center gap-2 rounded-lg border border-slate-500 bg-[#13233f] px-4 py-2 text-sm font-black text-white hover:border-orange-300" href="/job-sources">
      ...
      <section className="app-grid-3 grid gap-3 lg:grid-cols-3">
        {sources.map((source) => {
          const Icon = source.icon;
          return (
            <Link key={source.name} href={source.href} className="app-source flex items-center gap-3 rounded-xl border border-[#2a4063] bg-[#13233f] p-4 hover:border-orange-400">
              <span className="app-icon flex size-11 items-center justify-center rounded-lg bg-[#0c172b] text-orange-300">
                <Icon className="size-5" />
              </span>
              <span className="min-w-0 flex-1">
                <span className="flex flex-wrap items-center gap-2">
                  <span className="font-black text-white">{source.name}</span>
                  <span className={`app-badge rounded-full px-2 py-0.5 text-[11px] font-black uppercase ${source.connected ? "app-badge-ok bg-emerald-500/20 text-emerald-200" : "bg-slate-700 text-slate-100"}`}>
          ...
          {upcomingJobs.length ? (
            <div className="grid gap-3">
              {upcomingJobs.slice(0, 4).map((row) => (
                <Link className="app-row rounded-xl border border-[#2a4063] bg-[#0c172b] p-3 hover:border-orange-400" href={`/work-orders/${String(row.id)}`} key={String(row.id)}>
          ...
          {recentWorkOrders.length ? (
            <div className="grid gap-3">
              {recentWorkOrders.slice(0, 5).map((row) => (
                <Link className="app-row rounded-xl border border-[#2a4063] bg-[#0c172b] p-4 hover:border-orange-400" href={`/work-orders/${String(row.id)}`} key={String(row.id)}>

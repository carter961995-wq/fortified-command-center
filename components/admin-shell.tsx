...
export function AdminShell({ children, profile, envWarning }: { children: React.ReactNode; profile?: PlainRow | null; envWarning?: string }) {
  return (
    <div className="app-shell min-h-screen bg-[#081326] text-slate-100">
      <aside className="app-sidebar hidden w-[240px] border-r border-[#1c2b45] bg-[#060d1d] text-white shadow-2xl md:flex">
        <div className="border-b border-[#17243c] p-4">
          <Link href="/dashboard" className="app-brand block overflow-hidden rounded-lg border border-[#1e3152] bg-[#101d34] shadow-[0_16px_50px_rgba(0,0,0,0.35)]">
            <div className="app-brand-inner relative h-24 bg-[radial-gradient(circle_at_30%_15%,rgba(251,146,60,0.45),transparent_28%),linear-gradient(135deg,#1a2d4f,#080f21_60%,#3a1f0d)] p-3">
        ...
        <SidebarNav />
        <div className="app-sidebar-foot border-t border-[#17243c] bg-[#060d1d] p-4 text-xs text-slate-500">
          <p className="font-bold text-slate-200">{String(profile?.full_name ?? profile?.email ?? "Boss")}</p>
          <form action={signOutAction} className="mt-3">
            <button className="app-btn-link text-orange-400 hover:text-orange-300" type="submit">Sign out</button>
          </form>
        </div>
      </aside>
      <div className="app-content">
        ...
        {envWarning ? <div className="app-banner border-b border-orange-500/20 bg-orange-500/10 px-6 py-3 text-sm font-semibold text-orange-200">{envWarning}</div> : null}
        <main className="app-main min-h-screen bg-[#0b1629] p-4 sm:p-6 lg:p-8">{children}</main>
      </div>
    </div>
  );
}

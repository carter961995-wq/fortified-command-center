...
const JOB_EMAIL_HINTS =
  /\b(work\s*order|wo[#:\s-]|job\s*assigned|new\s*job|service\s*request|dispatch|store\s*#|dne|n\.?t\.?e\.?|not\s*to\s*exceed|mhelp|mhelpdesk|truesource|affiliate\s*connect|ticket\s*#)\b/i;
...
Email: dana.ruiz@example.com`;
  ...
  const trueSourceBody = `TrueSource Affiliate Connect assignment
...
Details: Two 8ft panels damaged by trailer swing. Replace panels and photo before/after.
DNE: $1450.00
...
Email: night.desk@nfn.example`;
  ...
  await upsertJobIntakeFromSource({
    source: "truesource",
    sourceRef: "seed-demo-ts-77419",
    subject: "TrueSource Affiliate Connect · TS-77419",
    from: "noreply@affiliate.truesource.example",
    snippet: "New Affiliate Connect work order at Regional Distribution Hub",
    rawText: trueSourceBody,
    parsed: heuristicParse(trueSourceBody, "TrueSource Affiliate Connect · TS-77419"),
  });
